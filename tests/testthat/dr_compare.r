...
if (PLOT) {
  ...
  if (args$technique == "umap") {
    min_dist_seq <- seq(from = 0.001, to = 0.05, by = 0.005)
    n_neighbors_seq <- seq(from = 5, to = 50, by = 10)
    for (min_dist in min_dist_seq) {
    ...
    arranged <- ggarrange(plotlist = results, common.legend = TRUE)
    ggsave("{args$figure_path}/umap_all.png", arranged)
