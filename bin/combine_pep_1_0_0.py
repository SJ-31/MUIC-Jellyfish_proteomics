...
def sliding_window(elements, window_size, flexible=True):
    ...
    start_gen, stop_gen = itertools.tee(
        elements
    )  # get 2 generators, one that tells
    ...
    n_decoys = (
        0  # keep track of the number of decoys in current sliding window
    )
    current_win_size = 0  # keep track of current window size
    previous_start_i, previous_stop_i = (
        0,
        0,
    )  # remember where our sliding window
...
class CombinedPEP(object):
    ...
    def naive_bayes(prob_list):
        ...
        return multiplied_probs / (
            multiplied_probs + multiplied_opposite_probs
        )
    ...
    def write_output_csv(self, output_csv_path):
        print(
            ...
            "and Bayes PEPs to\n{} ...\n".format(
                os.path.relpath(output_csv_path)
            )
        )
        new_scores = ["combined PEP", "Bayes PEP"]
        fieldnames = (
            list(self.input_csv_fieldnames)
            + new_scores
            + ["combined PEP engines"]
        )
