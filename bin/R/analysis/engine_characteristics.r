...
final_results <- "/results/jellyfish/1-First_pass/jellyfish_all.tsv"
percolator_dir <- "/results/jellyfish/1-First_pass/Percolator"
...
id_list <- lapply(engines, function(x) {
  return((tibbles %>% bind_rows() %>% filter(engine == x))$ProteinId)
}) %>% `names<-`(engines)
