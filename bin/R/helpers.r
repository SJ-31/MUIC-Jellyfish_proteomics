...
tb_duplicate_at <- function(tibble, col, separator, split_fn = NULL) {
  # Perform "row_duplicate_at" for to every applicable row in the tibble
  numeric_cols <- tibble %>%
  ...
  if (nrow(applicable) != 0) {
    applicable <- applicable %>%
      apply(., 1, \(x) {
        row_duplicate_at(x, column = col, sep = separator, split_fn = split_fn)
      }) %>%
      bind_rows()
  }
...
format_chi_expected <- function(table) {
  ...
  expected <- binary_table2tb(showExpectedChi(table)) |>
